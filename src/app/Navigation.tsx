...
const Navigation = () => {
    ...
    const [navIcon, setNavIcon] = useState("/img/menu.png");
    const [navView, setNavView] = useState("top-[-50rem]");

    const navToggle = () => {
        const iconChange = navIcon === "/img/x.png" ? "/img/menu.png" : "/img/x.png";
    ...
    return (
        <nav className={`h-[5.125rem] text-[#DAC5A7] flex items-center justify-between fixed w-full top-0 left-0 z-10 md:px-[5.81rem]   ${isScrolled ? "bg-[#000]" : "bg-transparent"} `}>
            ...
            <div className="hidden md:block">
                {theme === "dark" ? <div>
                    <button className=" p-2 rounded-full bg-[#FFF] " onClick={toggleTheme}><Image src="/img/eclipse.gif" width={40} height={20} alt="lightmode-sun" /></button>
                </div> : <div><button className=" p-2 rounded-full bg-[#FFF]" onClick={toggleTheme}> <Image src="/img/night-mode.gif" width={40} height={20} alt="darkmode-moon" /></button></div>}
            </div>
            <div className="bg-[#0E0E0E] h-full flex items-center px-[1rem] gap-[3.37rem] w-full justify-between md:w-fit">
                ...
                <div className=" ">
                    <div className="flex items-center justify-center hover:cursor-pointer md:hidden">
                        ...
                        <button onClick={navToggle}> <Image src={`${navIcon}`} width={40} height={20} alt="nav icon" /></button>
                    </div>
